 ...
 *
 * Kept in step with the DSR's copy of the same file: both portals clock people
 * in, and an answer that only one of them gives is an answer half the staff
 * never see.
 *
...
export function locationBlockedMessage(action: ClockAction = 'clock in'): string {
  ...
  if (isIOS()) {
    return 'Location is blocked for this app. Open Settings → Privacy & Security → Location Services, '
      + 'make sure it is on, then allow location for Timekeeper Online.';
  }
  if (/Android/.test(ua())) {
