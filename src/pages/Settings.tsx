...
function SalesTarget() {
  ...
  const [msg, setMsg] = useState<string | null>(null);
  useEffect(() => {
    supabase.from('settings').select('id, sales_target_month, sales_target_avenues, sales_target_timegallery, sales_target_online, sales_target_whatsapp').single().then(({ data }) => {
      if (data) {
        setId(data.id);
        ...
        setOnline(data.sales_target_online != null ? String(data.sales_target_online) : '');
        setWhatsapp(data.sales_target_whatsapp != null ? String(data.sales_target_whatsapp) : '');
      }
    });
  }, []);
  async function save() {
    ...
    const { error } = await supabase.from('settings').update({
      ...
      sales_target_online: num(online),
      sales_target_whatsapp: num(whatsapp),
    }).eq('id', id);
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 lg:col-span-2">
      ...
      <p className="text-xs text-slate-400 mb-3">Shown on the dashboard as “Sales vs target”, measured against till revenue from Lightspeed. Set an overall target and, if you like, one per channel. Online and WhatsApp share one till register and are told apart by who rang the sale up, so they take a target each. Leave a field blank to hide that card.</p>
      <div className="flex flex-wrap items-end gap-2">
        <label className="text-xs">
        ...
        <label className="text-xs">
          <span className="block text-slate-500 mb-1">Time Keeper Online (KD / month)</span>
          <input type="number" value={online} onChange={(e) => setOnline(e.target.value)} placeholder="e.g. 35000"
            className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm w-36" />
        </label>
        <label className="text-xs">
          <span className="block text-slate-500 mb-1">Time Keeper WhatsApp (KD / month)</span>
          <input type="number" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} placeholder="e.g. 15000"
            className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm w-36" />
        </label>
        <label className="text-xs">
          <span className="block text-slate-500 mb-1">Avenues (KD / month)</span>
          <input type="number" value={avenues} onChange={(e) => setAvenues(e.target.value)} placeholder="e.g. 30000"
            className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm w-36" />
        </label>
        <label className="text-xs">
          <span className="block text-slate-500 mb-1">Time Gallery (KD / month)</span>
          <input type="number" value={timeGallery} onChange={(e) => setTimeGallery(e.target.value)} placeholder="e.g. 20000"
            className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm w-36" />
        </label>
        <button onClick={save} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-slate-900 text-white text-sm font-medium hover:bg-slate-700"><Save size={13} /> Save targets</button>
        {msg && <span className={`text-xs ${msg === 'Targets saved' ? 'text-emerald-600' : 'text-red-600'}`}>{msg}</span>}
      </div>
    </div>
  );
}
