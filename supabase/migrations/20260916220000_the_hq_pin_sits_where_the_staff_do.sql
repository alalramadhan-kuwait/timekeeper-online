...
-- Four July rows predate the trigger and carry coordinates but no workplace,
-- so the re-measure above skips them and their distance stays stale. Each one
-- sits inside a single fence; name it and measure it like the rest.
update public.attendance_records a
   set location = g.name,
       clock_in_distance_m = round(geo_distance_m(a.clock_in_lat, a.clock_in_lng, g.lat, g.lng))
  from public.geofences g
 where a.location is null
   and a.clock_in_lat is not null
   and g.active
   and geo_distance_m(a.clock_in_lat, a.clock_in_lng, g.lat, g.lng) <= g.radius_m;
